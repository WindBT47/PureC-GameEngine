	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		for (auto tri : meshCube.tris)
		{
			...
			if (Vector_DotProduct(normal, vCameraRay) < 0.0f)
			{
				...
				float dp = max(0.1f, Vector_DotProduct(normal, light_direction));

				// Choose Console Colors As Required
				...
				// Clip Viewed Triangle Against Near Plane, This Could From Two Additional
		...
		for (auto& triToRaster : vecTrianglesToRaster)
		{
			...
			for (int p = 0; p < 4; p++)
			{
				...
				while (nNewTriangles > 0)
				{
					...
					// So Add These New Ones To The Back Of The Queue For Subsequent
